...
begin
  if exists (select 1 from information_schema.tables
              where table_schema = 'public' and table_name = 'reposts') then
    update public.posts p
       set repost_count = coalesce(
         (select count(*) from public.reposts r where r.post_id = p.id), 0);
  end if;
end $$;
...
-- ─── Репосты ────────────────────────────────────────────────────────────────
--
-- Таблица появляется миграцией 006; у кого её нет, триггер просто не создаётся.
do $$
begin
  ...
  execute 'drop trigger if exists reposts_post_count on public.reposts';
  execute 'create trigger reposts_post_count
