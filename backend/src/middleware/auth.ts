...
async function userState(userId: string): Promise<UserState> {
  ...
  /**
   * Заодно отмечаем, что человека видели.
   *
   * Здесь, а не отдельным обращением на каждый запрос: кеш обновляется раз в
   * минуту, и запись получается ровно такой же частоты. Писать на каждый
   * запрос значило бы обновлять строку пользователя по десятку раз на одном
   * открытии ленты — ради числа, которое смотрят раз в день.
   *
   * Не ждём ответа и молчим об ошибке: «в сети» — украшение статистики, а не
   * то, ради чего стоит задерживать ответ или ронять запрос. Колонки может и
   * не быть вовсе, пока не выполнена миграция 028.
   */
  void supabase
    .from('users')
