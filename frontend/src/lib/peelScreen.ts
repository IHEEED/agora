 ...
 * Сроки переходов.
 *
 * Убавлены примерно на четверть от прежних. Это не погоня за числом: пока
 * длительности подбирались на настольной машине, они казались точными, а на
 * телефоне между нажатием и результатом добавляется своя задержка — отклик
 * касания, отрисовка, сеть, — и поверх неё честные 220 мс ощущаются
 * заторможенностью. Убрать эту задержку нельзя, поэтому убавляем то, чем
 * распоряжаемся сами.
 *
 * Ниже этих значений идти не стоит: короче 120 мс движение перестаёт читаться
 * движением и превращается в подмену кадра — а именно ради связи «откуда куда»
 * оно и существует.
 */
