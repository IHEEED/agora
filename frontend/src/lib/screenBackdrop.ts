...
export function holdBackdrop() {
  ...
  layer.style.cssText = [
    ...
    'pointer-events:none',
    /**
     * Непрозрачная подложка обязательна.
     *
     * Слой позиционирован, то есть рисуется поверх живой страницы. Экраны в
     * приложении прозрачны — фон рисует страница, — и без своей заливки сквозь
     * снимок просвечивала настоящая лента: текст в две копии со сдвигом,
     * карточки поверх сториз. Особенно заметно на возврате, когда живая лента
     * уже отрисована на нулевой прокрутке, а копия помнит прежнюю.
     *
     * Размытию это не мешает: затемнение шторки лежит выше и размывает всё, что
     * под ним, вместе с заливкой.
     */
    'background:var(--bg)',
  ].join(';');
