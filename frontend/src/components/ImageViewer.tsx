...
export function ImageViewer({
  ...
  function onPointerMove(event: React.PointerEvent) {
    ...
    // Одна картинка вбок не ходит.
    //
    // Раньше горизонталь принималась всегда, и единственный снимок ездил за
    // пальцем в никуда: листать некуда, поэтому он просто уезжал и возвращался.
    // Со стороны это и читалось как «прыгает не пойми куда».
    if (axis.current === 'x') offset.current = { x: images.length > 1 ? dx : 0, y: 0 };
  ...
  function onPointerUp() {
    ...
    /**
     * Один жест — один кадр, как бы резко ни смахнули.
     *
     * go() и раньше сдвигал на единицу, но порог был единственным условием, и
     * при резком броске жест успевал отработать дважды: pointerup приходил, а
     * инерция страницы догоняла вторым событием. Отсюда «механика превращается
     * не пойми во что». Теперь дальше единицы не уходим никогда — то же
     * правило, что у ленты снимков в записи (scroll-snap-stop).
     */
    const threshold = window.innerWidth * 0.25;
    if (images.length > 1 && committed(x, vx, threshold)) {
      go(x < 0 ? 1 : -1);
