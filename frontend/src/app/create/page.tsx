 ...
 *
 * Здесь стояло 300 — число из тех, что пишут, чтобы «ограничение было». Триста
 * знаков это не заголовок, а абзац: в ленте он набран в 16 пунктов и на узком
 * экране занял бы восемь строк, отодвинув и текст, и снимки за нижнюю кромку.
 *
 * Восемьдесят — примерно три строки на телефоне. Три строки заголовок ещё
 * держит: его читают одним взглядом. Четвёртая превращает его в начало текста,
 * и тогда непонятно, зачем он вообще отдельный.
 *
 * Обрезать в ленте многоточием было бы хуже предела: обрезка прячет то, что
 * человек уже написал и считает написанным, — то есть врёт ему о его же
 * записи. Предел честнее, потому что виден заранее.
 */
const TITLE_MAX = 80;
...
function CreatePost() {
  ...
  return sheet(
    ...
    >
        ...
        {/* Счётчик появляется только на подходе к пределу.
            Висящий всегда, он превращает написание записи в заполнение бланка:
            человек начинает следить за числом вместо того, чтобы думать о
            заголовке. А когда места действительно осталось мало, молчать
            нельзя — упереться в предел на полуслове хуже, чем увидеть его
            заранее. */}
        {title.length > TITLE_MAX - 20 && (
          <p
            className="font-num -mt-1 px-1 text-right text-[12px]"
            style={{
              color: title.length >= TITLE_MAX ? 'var(--down)' : 'var(--text-muted)',
            }}
          >
