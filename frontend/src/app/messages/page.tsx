...
export default function MessagesPage() {
  ...
  /**
   * Обычный уход, а не складывание в кнопку.
   *
   * Мессенджер разворачивался из своего значка в шапке и складывался обратно в
   * него. Приём красивый и в одном месте уместный — поиск действительно растёт
   * из лупы, — но здесь он давал ровно то, на что жаловались: экран будто
   * вылетает снизу и всасывается обратно. Мессенджер не всплывающее окно, а
   * такой же раздел, как лента: между разделами переходят, а не разворачивают
   * их из точки.
   *
   * Селектор кнопки убран — с ним useScreenLeave выбирает складывание.
   */
  const { style: leaveStyle, swipeHandlers } = useScreenLeave();
